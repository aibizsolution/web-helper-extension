...
## 현재 사용자 기능
- `번역`
  - `사이트`: API Key 없이 빠르게 읽는 초고속 번역과 AI 정밀 번역 제공
...
### 선택 텍스트 번역
- 드래그 액션바와 우클릭 메뉴 모두 `번역 / 복사 / 설명하기` 기능 축으로 맞춘다
...
- 이 저장소는 현재 `package.json`이 없으므로 `npm install` 기반 온보딩을 전제로 설명하지 않음
- 새 PC 온보딩은 기본적으로 `clone -> 압축해제된 확장 프로그램 로드 -> 필요 시 Node 검증` 흐름으로 안내
- 확장 설정값과 API Key는 `chrome.storage.local`에 저장되므로 Git으로 PC 간 이동되지 않음을 항상 명시
